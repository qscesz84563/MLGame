    ...
    while True:
        ...
        if scene_info["ball_speed"][1] > 0 and scene_info["ball"][1] <= 240:
            if(scene_info["ball_speed"][1] < 0):
                x = abs(scene_info["ball"][0] // scene_info["ball_speed"][0])
                pred_y = scene_info["ball"][1] + scene_info["ball_speed"][1] * x
                ball_x = 0
                ball_vx = scene_info["ball_speed"][0] * -1
                x = abs((scene_info["blocker"][0] - ball_x) // ball_vx)
                pred_y = pred_y + scene_info["ball_speed"][1] * x
                if(pred_y > 240 and pred_y < 260):
                    ball_x = scene_info["blocker"][1]
                    ball_vx *= -1
                    y = abs((scene_info["platform_1P"][1] - pred_y) // scene_info["ball_speed"][1])
                    pred = ball_x + ball_vx * y
                ...
                x = abs((200 - scene_info["ball"][0]) // scene_info["ball_speed"][0])
                pred_y = scene_info["ball"][1] + scene_info["ball_speed"][1] * x
                ball_x = 200
                ball_vx = scene_info["ball_speed"][0] * -1
                x = abs(((scene_info["blocker"][0]+30) - ball_x) // ball_vx)
                pred_y = pred_y + scene_info["ball_speed"][1] * x
                if(pred_y > 240 and pred_y < 260):
                    ball_x = scene_info["blocker"][1]
                    ball_vx *= -1
            ...
            if (bound > 0):
                ...
                if bound%2 ==1:
                    ...
                    pred = pred + (abs(bound)*200)
        elif(scene_info["ball"][1] >= 240 and scene_info["ball_speed"][1] < 0):
